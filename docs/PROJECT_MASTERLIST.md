...
Diese Bestands- und Backlogliste ist keine Task-Autorisierung. Ein funktionaler Task darf ausschließlich durch den Projekteigentümer und den verbindlichen Projektchat ausgewählt und in `docs/tasks/CURRENT_TASK.md` ausdrücklich autorisiert werden. `CURRENT_TASK` ist aktuell `BLOCKED`; aus dieser Liste ist kein Folgeschritt ausgewählt.
