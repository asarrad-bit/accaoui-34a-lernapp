...
const registryReadCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState:
      registryReadResultAcceptance,
    privateField:
      "nicht übernehmen"
...
assert(
  registryReadCompletion.status ===
    "exam_result_history_persistence_cycle_registry_completion_read_ready" &&
  registryReadCompletion.isValid ===
    true &&
  registryReadCompletion.canFinalizePersistence ===
    true &&
  registryReadCompletion.isTerminal ===
    true &&
  registryReadCompletion.isSuccessful ===
    true &&
  registryReadCompletion.terminalOutcome ===
    "read_ready" &&
  registryReadCompletion.canUseRegistry ===
    true &&
  registryReadCompletion.didRead ===
    true &&
  registryReadCompletion.completedCycleCount ===
    1 &&
  registryReadCompletion.registryState.isValid ===
    true &&
  registryReadCompletion.canExecuteStorage ===
    false &&
  registryStorageAdapterOperationCalls ===
    0,
  "Zyklusregister-Read-Abschluss wurde nicht sicher erstellt"
);

expectJson(
  registryReadCompletion.registryPayload,
  registeredReadCycleRegistry.registryPayload,
  "Abgeschlossener Zyklusregister-Read-Payload"
);

assert(
  !Object.prototype.hasOwnProperty.call(
    registryReadCompletion,
    "acceptanceState"
  ) &&
  !Object.prototype.hasOwnProperty.call(
    registryReadCompletion,
  ...
  "Interne Zyklusregister-Abschlussfelder wurden übernommen"
);

const registryReadEmptyCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState:
      registryReadEmptyResultAcceptance
  });

assert(
  registryReadEmptyCompletion.status ===
    "exam_result_history_persistence_cycle_registry_completion_read_empty" &&
  registryReadEmptyCompletion.isTerminal ===
    true &&
  registryReadEmptyCompletion.terminalOutcome ===
    "read_empty" &&
  registryReadEmptyCompletion.didRead ===
    true &&
  registryReadEmptyCompletion.isEmpty ===
    true &&
  registryReadEmptyCompletion.canUseRegistry ===
    false,
  "Leerer Zyklusregister-Read-Abschluss wurde nicht erstellt"
);

const registryWriteCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState:
      registryWriteResultAcceptance
  });

assert(
  registryWriteCompletion.status ===
    "exam_result_history_persistence_cycle_registry_completion_write_confirmed" &&
  registryWriteCompletion.isTerminal ===
    true &&
  registryWriteCompletion.terminalOutcome ===
    "write_confirmed" &&
  registryWriteCompletion.didWrite ===
    true &&
  registryWriteCompletion.completedCycleCount ===
    registryWriteResultAcceptance.completedCycleCount &&
  registryWriteCompletion.serializedByteLength ===
    registryWriteResultAcceptance.serializedByteLength,
  "Zyklusregister-Write-Abschluss wurde nicht erstellt"
);

const registryDeleteCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState:
      registryDeleteResultAcceptance
  });

assert(
  registryDeleteCompletion.status ===
    "exam_result_history_persistence_cycle_registry_completion_delete_confirmed" &&
  registryDeleteCompletion.isTerminal ===
    true &&
  registryDeleteCompletion.terminalOutcome ===
    "delete_confirmed" &&
  registryDeleteCompletion.didDelete ===
    true &&
  registryDeleteCompletion.wasAlreadyAbsent ===
    false,
  "Zyklusregister-Delete-Abschluss wurde nicht erstellt"
);

const registryDeleteAbsentCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState:
      registryDeleteAbsentResultAcceptance
  });

assert(
  registryDeleteAbsentCompletion.status ===
    "exam_result_history_persistence_cycle_registry_completion_delete_absent" &&
  registryDeleteAbsentCompletion.isTerminal ===
    true &&
  registryDeleteAbsentCompletion.terminalOutcome ===
    "delete_absent" &&
  registryDeleteAbsentCompletion.didDelete ===
    true &&
  registryDeleteAbsentCompletion.wasAlreadyAbsent ===
    true,
  "Bereits fehlendes Zyklusregister wurde nicht terminal abgeschlossen"
);

const staleRegistryCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState:
      staleRegistryResultAcceptance
  });

expectEqual(
  staleRegistryCompletion.reason,
  "persistence_cycle_registry_completion_acceptance_invalid",
  "Veraltete Zyklusregister-Ergebnisannahme"
);

const tamperedRegistryCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState({
    acceptanceState: {
      ...registryReadResultAcceptance,
      resultAcceptanceIdentity:
        "exam_history_persistence_cycle_registry_result_acceptance:save:v1"
    }
  });

expectEqual(
  tamperedRegistryCompletion.reason,
  "persistence_cycle_registry_completion_identity_invalid",
  "Manipulierte Zyklusregister-Ergebnisannahme-Identität"
);

let registryCompletionAccessorReads = 0;

const registryCompletionAccessorInput = {};

Object.defineProperty(
  registryCompletionAccessorInput,
  "acceptanceState",
  {
    enumerable: true,
    get() {
      registryCompletionAccessorReads +=
        1;

      throw new Error(
        "Zyklusregister-Abschluss darf Getter nicht ausführen."
      );
    }
  }
);

const accessorRegistryCompletion =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCompletionState(
    registryCompletionAccessorInput
  );

assert(
  accessorRegistryCompletion.reason ===
    "persistence_cycle_registry_completion_acceptance_accessor_not_allowed" &&
  registryCompletionAccessorReads ===
    0 &&
  registryStorageAdapterOperationCalls ===
    0,
  "Zyklusregister-Abschluss-Getter wurde nicht blockiert"
);

const registryReadCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryLoadInvocationPackage,
    resultContractState:
      registryReadResultContract,
...
assert(
  registryReadCycle.status ===
    "exam_result_history_persistence_cycle_registry_cycle_completed" &&
  registryReadCycle.isValid ===
    true &&
  registryReadCycle.canFinalizeCycle ===
    true &&
  registryReadCycle.isTerminal ===
    true &&
  registryReadCycle.isSuccessful ===
    true &&
  registryReadCycle.canUseRegistry ===
    true &&
  registryReadCycle.intent ===
    "load" &&
  registryReadCycle.operation ===
    "read" &&
  registryReadCycle.terminalOutcome ===
    "read_ready" &&
  registryReadCycle.didRead ===
    true &&
  registryReadCycle.completedCycleCount ===
    1 &&
  registryReadCycle.registryState.isValid ===
    true &&
  registryReadCycle.canExecuteStorage ===
    false &&
  registryStorageAdapterOperationCalls ===
    0,
  "Zyklusregister-Read-Zyklus wurde nicht sicher erstellt"
);

expectJson(
  registryReadCycle.registryPayload,
  registeredReadCycleRegistry.registryPayload,
  "Abgeschlossener Zyklusregister-Read-Zyklus"
);
...
assert(
  ...
  "Interne Zyklusregister-Zyklusfelder wurden übernommen"
);

const registryReadEmptyCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryLoadInvocationPackage,
    resultContractState:
    ...
    acceptanceState:
      registryReadEmptyResultAcceptance,
    completionState:
      registryReadEmptyCompletion
  });

assert(
  registryReadEmptyCycle.status ===
    "exam_result_history_persistence_cycle_registry_cycle_completed" &&
  registryReadEmptyCycle.terminalOutcome ===
    "read_empty" &&
  registryReadEmptyCycle.didRead ===
    true &&
  registryReadEmptyCycle.isEmpty ===
    true &&
  registryReadEmptyCycle.canUseRegistry ===
    false,
  "Leerer Zyklusregister-Read-Zyklus wurde nicht erstellt"
);

const registryWriteCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registrySaveInvocationPackage,
    resultContractState:
    ...
    acceptanceState:
      registryWriteResultAcceptance,
    completionState:
      registryWriteCompletion
  });

assert(
  registryWriteCycle.status ===
    "exam_result_history_persistence_cycle_registry_cycle_completed" &&
  registryWriteCycle.intent ===
    "save" &&
  registryWriteCycle.operation ===
    "write" &&
  registryWriteCycle.terminalOutcome ===
    "write_confirmed" &&
  registryWriteCycle.didWrite ===
    true &&
  registryWriteCycle.completedCycleCount ===
    registryWriteCompletion.completedCycleCount,
  "Zyklusregister-Write-Zyklus wurde nicht erstellt"
);

const registryDeleteCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryDeleteInvocationPackage,
    resultContractState:
    ...
    acceptanceState:
      registryDeleteResultAcceptance,
    completionState:
      registryDeleteCompletion
  });

assert(
  registryDeleteCycle.status ===
    "exam_result_history_persistence_cycle_registry_cycle_completed" &&
  registryDeleteCycle.intent ===
    "delete" &&
  registryDeleteCycle.terminalOutcome ===
    "delete_confirmed" &&
  registryDeleteCycle.didDelete ===
    true &&
  registryDeleteCycle.wasAlreadyAbsent ===
    false,
  "Zyklusregister-Delete-Zyklus wurde nicht erstellt"
);

const registryDeleteAbsentCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryDeleteInvocationPackage,
    resultContractState:
    ...
    acceptanceState:
      registryDeleteAbsentResultAcceptance,
    completionState:
      registryDeleteAbsentCompletion
  });

assert(
  registryDeleteAbsentCycle.status ===
    "exam_result_history_persistence_cycle_registry_cycle_completed" &&
  registryDeleteAbsentCycle.terminalOutcome ===
    "delete_absent" &&
  registryDeleteAbsentCycle.didDelete ===
    true &&
  registryDeleteAbsentCycle.wasAlreadyAbsent ===
    true,
  "Bereits fehlender Zyklusregister-Zyklus wurde nicht abgeschlossen"
);

const staleRegistryCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryLoadInvocationPackage,
    resultContractState:
...
expectEqual(
  staleRegistryCycle.reason,
  "persistence_cycle_registry_cycle_recomputed_acceptance_not_ready",
  "Veralteter Zyklusregister-Zyklus"
);

const tamperedRegistryAcceptanceCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryLoadInvocationPackage,
    resultContractState:
...
expectEqual(
  tamperedRegistryAcceptanceCycle.reason,
  "persistence_cycle_registry_cycle_acceptance_state_mismatch",
  "Manipulierte Zyklusregister-Ergebnisannahme im Zyklus"
);

const tamperedRegistryCompletionCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState({
    invocationPackageState:
      registryLoadInvocationPackage,
    resultContractState:
...
expectEqual(
  tamperedRegistryCompletionCycle.reason,
  "persistence_cycle_registry_cycle_completion_state_mismatch",
  "Manipulierter Zyklusregister-Abschluss im Zyklus"
);

let registryCycleAccessorReads = 0;

const registryCycleAccessorInput = {
  invocationPackageState:
    registryLoadInvocationPackage,
  resultContractState:
    registryReadResultContract,
...
Object.defineProperty(
  registryCycleAccessorInput,
  "completionState",
  {
    enumerable: true,
    get() {
      registryCycleAccessorReads +=
        1;

      throw new Error(
        "Zyklusregister-Zyklus darf Getter nicht ausführen."
      );
    }
  }
);

const accessorRegistryCycle =
  adapter.mapParticipantFullExamResultHistorySnapshotPersistenceCycleRegistryCycleState(
    registryCycleAccessorInput
  );

assert(
  accessorRegistryCycle.reason ===
...
console.log(
  "Zyklusregister-Abschluss-Fixtures: Read, leer, Write, Delete, nicht vorhanden, veraltet, manipuliert und Accessor"
);
console.log(
