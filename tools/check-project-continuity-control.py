...
def extract_v2737a_current_task_header(task_text: str) -> str:
    heading_matches = re.findall(
        rf"(?m)^{re.escape(V2737A_CURRENT_TASK_HEADER_MARKER)}$",
        task_text,
    )
    require(
    ...
    tail = task_text[len(V2737A_CURRENT_TASK_HEADER_MARKER):]
    next_heading = re.search(r"(?m)^## ", tail)
    require(
...
V2737B_GATE_BOOTSTRAP_REPAIR_FILES = V2737B_BOOTSTRAP_FILES
...
V2737B_IMPLEMENTATION_FILE_ORDER = (
...
V2737B_IMPLEMENTATION_FILES = frozenset(V2737B_IMPLEMENTATION_FILE_ORDER)
...
V2737B_BASE_CLOSED_TASK_FIELDS = {
    "Task-ID": "NONE",
    "Status": "BLOCKED",
    "Autorisiert": "NEIN",
    "Titel": "Kein Task autorisiert",
...
def replace_v2737b_exact_once(
    text: str, needle: str, replacement: str
) -> str:
    require(
        text.count(needle) == 1,
        f"v27.37b-Manipulationsziel ist nicht exakt einmal vorhanden: {needle}",
    )
    mutated = text.replace(needle, replacement, 1)
    require(
        mutated != text and needle not in mutated,
        f"v27.37b-Manipulation hat das Ziel nicht entfernt: {needle}",
    )
    return mutated


def insert_v2737b_after_exact_once(
    text: str, anchor: str, addition: str
) -> str:
    require(
        text.count(anchor) == 1,
        f"v27.37b-Additivanker ist nicht exakt einmal vorhanden: {anchor}",
    )
    require(
        bool(addition) and addition not in text,
        f"v27.37b-Additivtext ist leer oder bereits vorhanden: {addition}",
    )
    mutated = text.replace(anchor, anchor + addition, 1)
    require(
        mutated != text
        and mutated.count(anchor) == 1
        and mutated.count(addition) == 1,
        f"v27.37b-Additivmutation war nicht eindeutig: {addition}",
    )
    return mutated


def replace_v2737b_in_delimited_block(
    text: str,
    block_start: str,
    block_end: str,
    needle: str,
    ...
    require(
        text.count(block_start) == 1 and text.count(block_end) == 1,
        "v27.37b-Manipulationsblock ist nicht eindeutig abgegrenzt",
    )
    prefix, tail = text.split(block_start, 1)
    block, suffix = tail.split(block_end, 1)
    mutated_block = replace_v2737b_exact_once(block, needle, replacement)
    return prefix + block_start + mutated_block + block_end + suffix


def insert_v2737b_after_in_delimited_block(
    text: str,
    block_start: str,
    block_end: str,
    anchor: str,
    ...
    require(
        text.count(block_start) == 1 and text.count(block_end) == 1,
        "v27.37b-Additivblock ist nicht eindeutig abgegrenzt",
    )
    prefix, tail = text.split(block_start, 1)
    block, suffix = tail.split(block_end, 1)
    mutated_block = insert_v2737b_after_exact_once(block, anchor, addition)
    return prefix + block_start + mutated_block + block_end + suffix


def extract_v2737b_bootstrap_section(text: str, document_name: str) -> str:
    require(
        text.count(V2737B_BOOTSTRAP_SECTION_HEADING) == 1,
        f"{document_name}: v27.37b-GATE-BOOTSTRAP-Abschnitt fehlt oder ist doppelt",
    )
...
def validate_v2737b_bootstrap_section(section: str, document_name: str) -> None:
    for marker in V2737B_BOOTSTRAP_REQUIRED_MARKERS:
        require(
            marker in section,
            f"{document_name}: v27.37b-GATE-BOOTSTRAP-Pflichtaussage fehlt: {marker}",
        )
    ...
    require(
        section.count(bootstrap_start) == 1 and section.count(bootstrap_end) == 1,
        f"{document_name}: v27.37b-Bootstrap-Dateiliste ist nicht eindeutig",
    )
    ...
    require(
        bootstrap_list
        == "\n".join(f"- `{path}`" for path in V2737B_BOOTSTRAP_FILE_ORDER),
        f"{document_name}: v27.37b-Bootstrap-Dateiliste muss exakt sechs Dateien enthalten",
    )
    ...
    require(
        section.count(implementation_start) == 1
        and section.count(implementation_end) == 1,
        f"{document_name}: späterer v27.37b-Implementierungsscope ist nicht eindeutig",
    )
    implementation_list = section.split(implementation_start, 1)[1].split(
    ...
    require(
        ...
        f"{document_name}: späterer v27.37b-Implementierungsscope muss exakt vier Dateien enthalten",
    )
    factory_contract_end = "Gültige methodenspezifische v27.37a-Ergebnisse"
    require(
        section.count(factory_contract_end) == 1,
        f"{document_name}: v27.37b-Factory-Vertrag ist nicht eindeutig abgegrenzt",
    )
    factory_contract = (
        implementation_end
    ...
    require(
        factory_contract == V2737B_FACTORY_CONTRACT,
        f"{document_name}: v27.37b-Factory-Vertrag muss exakt zwei Dependencies und drei öffentliche Methoden enthalten",
    )
    shas = frozenset(re.findall(r"\b[0-9a-f]{40}\b", section))
    require(
        shas == frozenset({V2737B_GATE_BOOTSTRAP_BASE_SHA}),
        f"{document_name}: stabile v27.37b-Basis fehlt oder zukünftige SHA ist hartcodiert",
    )


def validate_v2737b_bootstrap_documents(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
    ...
    for text, name in (
        ...
        validate_v2737b_bootstrap_section(
            extract_v2737b_bootstrap_section(text, name),
            name,
        )


def extract_v2737b_gate_bootstrap_repair_section(
    text: str, document_name: str
) -> str:
    require(
        text.count(V2737B_GATE_BOOTSTRAP_REPAIR_SECTION_HEADING) == 1,
        f"{document_name}: v27.37b-GATE-BOOTSTRAP-REPAIR-Abschnitt fehlt oder ist doppelt",
    )
...
def validate_v2737b_gate_bootstrap_repair_section(
    section: str, document_name: str
) -> None:
    for marker in V2737B_GATE_BOOTSTRAP_REPAIR_REQUIRED_MARKERS:
        require(
            marker in section,
            f"{document_name}: v27.37b-GATE-BOOTSTRAP-REPAIR-Pflichtaussage fehlt: {marker}",
        )
    ...
    require(
        ...
        f"{document_name}: v27.37b-GATE-BOOTSTRAP-REPAIR-Dateiliste ist nicht eindeutig",
    )
    ...
    require(
        file_list
        == "\n".join(f"- `{path}`" for path in V2737B_BOOTSTRAP_FILE_ORDER),
        f"{document_name}: v27.37b-GATE-BOOTSTRAP-REPAIR muss exakt sechs Kontrollinfrastruktur-Dateien enthalten",
    )
    shas = frozenset(re.findall(r"\b[0-9a-f]{40}\b", section))
    require(
        shas == frozenset({V2737B_GATE_BOOTSTRAP_REPAIR_BASE_SHA}),
        f"{document_name}: Repair-Basis fehlt oder zukünftige Repair-SHA ist hartcodiert",
    )


def validate_v2737b_gate_bootstrap_repair_documents(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
...
def extract_v2737b_current_task_header(task_text: str) -> str:
    heading_matches = re.findall(
        rf"(?m)^{re.escape(V2737A_CURRENT_TASK_HEADER_MARKER)}$",
        task_text,
    )
...
def v2737b_current_task_state(task_text: str) -> str:
    header = extract_v2737b_current_task_header(task_text)
    ...
    if tuple(field_names) != V2737A_CURRENT_TASK_FIELD_NAMES:
        return "invalid"
    if fields == V2737B_BASE_CLOSED_TASK_FIELDS:
        return "v2737a_closed"
...
def synthetic_v2737b_current_task_state(task_text: str) -> str:
    try:
        return v2737b_current_task_state(task_text)
    except ValidationError:
        return "invalid"


def v2737b_expected_current_task_fields_for_phase(
    phase: str,
) -> dict[str, str]:
    phase_groups = {
        "v2737b_gate_bootstrap_prepared": V2737B_BASE_CLOSED_TASK_FIELDS,
        "v2737b_gate_bootstrap_committed": V2737B_BASE_CLOSED_TASK_FIELDS,
        "v2737b_gate_bootstrap_repair_prepared": V2737B_BASE_CLOSED_TASK_FIELDS,
        "v2737b_gate_bootstrap_repair_committed": V2737B_BASE_CLOSED_TASK_FIELDS,
    ...
    require(
        phase in phase_groups,
        f"Unbekannte v27.37b-Phase ohne CURRENT_TASK-Vertrag: {phase}",
    )
    return dict(phase_groups[phase])


def build_v2737b_synthetic_current_task(fields: dict[str, str]) -> str:
    require(
        tuple(fields) == V2737A_CURRENT_TASK_FIELD_NAMES,
        "Synthetische v27.37b-CURRENT_TASK-Felder sind unvollständig oder ungeordnet",
    )
    return (
        V2737A_CURRENT_TASK_HEADER_MARKER
        + "\n\n"
...
def synthetic_v2737b_bootstrap_document_is_valid(
    document_text: str, document_name: str
) -> bool:
    try:
        validate_v2737b_bootstrap_section(
            extract_v2737b_bootstrap_section(document_text, document_name),
            document_name,
        )
    except ValidationError:
        return False
...
def duplicate_v2737b_exact_once(text: str, needle: str) -> str:
    require(
        text.count(needle) == 1,
        f"v27.37b-Duplikationsziel ist nicht exakt einmal vorhanden: {needle}",
    )
    mutated = text.replace(needle, needle + "\n" + needle, 1)
    require(
        mutated != text and mutated.count(needle) == 2,
        f"v27.37b-Duplikation war nicht eindeutig: {needle}",
    )
    return mutated


def v2737b_scope_facts_are_valid(
    *,
    phase: str,
    task_state: str,
    working_files: frozenset[str],
    ...
    expected = {
        ...
        "v2737b_gate_bootstrap_committed": (
            "v2737a_closed", frozenset(), ("v2737b_bootstrap",),
        ),
        "v2737b_gate_bootstrap_repair_prepared": (
            "v2737a_closed",
        ...
        "v2737b_authorization_committed": (
            "v2737b_authorized", frozenset(),
            ("v2737b_bootstrap", "v2737b_bootstrap_repair", "v2737b_gate"),
        ),
        "v2737b_implementation_prepared": (
            "v2737b_authorized", V2737B_IMPLEMENTATION_FILES,
            ("v2737b_bootstrap", "v2737b_bootstrap_repair", "v2737b_gate"),
        ),
        "v2737b_implementation_committed": (
            "v2737b_authorized", frozenset(),
    ...
    if phase == "v2737b_authorization_prepared":
        return (
            task_state == "v2737b_authorized"
            and bool(working_files)
            and working_files <= V2737B_GATE_FILES
            and history_roles
            == ("v2737b_bootstrap", "v2737b_bootstrap_repair")
        )
    return expected.get(phase) == (task_state, working_files, history_roles)


def validate_v2737b_frozen_products() -> None:
    for relative_path in V2737B_FROZEN_PRODUCT_FILES:
        current_path = ROOT / relative_path
        require(
            current_path.is_file(),
            f"v27.37b-Frozen-Produktdatei fehlt: {relative_path}",
        )
        baseline = run_git_bytes(
            ["show", f"{V2737B_GATE_BOOTSTRAP_BASE_SHA}:{relative_path}"]
        )
        require(
            current_path.read_bytes() == baseline,
            f"v27.37b-Frozen-Produktdatei verändert: {relative_path}",
        )


def validate_v2737a_closed_at_v2737b_base() -> tuple[tuple[str, ...], str]:
    control_paths = (
        "docs/PROJECT_STATE_CURRENT.md",
        "docs/tasks/CURRENT_TASK.md",
        "docs/CURSOR_MASTER_CONTEXT_ACCAOUI.md",
    ...
    documents = tuple(
        read_v2735f_commit_document(V2737B_GATE_BOOTSTRAP_BASE_SHA, path)
        for path in control_paths
    )
    roles, implementation_commit = read_v2737a_history(
        V2737B_GATE_BOOTSTRAP_BASE_SHA
    )
    require(
        roles == ("v2737a_gate", "v2737a_implementation", "v2737a_closure")
        and implementation_commit is not None,
        "v27.37a ist an der v27.37b-Bootstrap-Basis nicht vollständig abgeschlossen",
    )
    validate_v2737a_completion_documents(
        *documents,
        implementation_commit,
    ...
    require(
        v2737a_current_task_state(documents[1]) == "v2737a_closed",
        "v27.37a-CURRENT_TASK ist an der Bootstrap-Basis nicht geschlossen",
    )
...
def read_v2737b_history(head: str) -> tuple[tuple[str, ...], str | None]:
    commits = tuple(
        line.strip()
        for line in run_git(
            ["rev-list", "--reverse", f"{V2737B_GATE_BOOTSTRAP_BASE_SHA}..{head}"]
        ).splitlines()
        if line.strip()
    )
    ...
    for commit in commits:
        ...
        require(
            len(lineage) == 2 and lineage[1] == previous,
            f"v27.37b-Commit muss direkt und mergefrei auf seinem Vorgänger liegen: {commit}",
        )
        files = frozenset(
            line.strip().replace("\\", "/")
            for line in run_git(
        ...
        if (
            ...
            require(
                commit == V2737B_GATE_BOOTSTRAP_REPAIR_BASE_SHA,
                "v27.37b-GATE-BOOTSTRAP-Commit stimmt nicht mit der stabilen Repair-Basis überein",
            )
            documents = tuple(
            ...
            validate_v2737b_bootstrap_documents(*documents)
            ...
            validate_v2737b_gate_bootstrap_repair_documents(*documents)
            ...
            roles
            == ["v2737b_bootstrap", "v2737b_bootstrap_repair", "v2737b_gate"]
            and files == V2737B_IMPLEMENTATION_FILES
            and task_state == "v2737b_authorized"
        ):
            ...
            raise ValidationError(
                f"Unbekannter, wiederholter oder falsch geordneter v27.37b-Commit: {commit}"
            )
        previous = commit
    return tuple(roles), implementation_commit


def validate_v2737b_lifecycle(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
) -> str:
    validate_v2737b_bootstrap_documents(
        state_text,
        task_text,
        cursor_text,
        masterlist_text,
    )
    validate_v2737b_frozen_products()
    branch = run_git(["branch", "--show-current"]).strip()
    head = run_git(["rev-parse", "HEAD"]).strip()
    origin_main = run_git(["rev-parse", "origin/main"]).strip()
    require(branch == "main", "v27.37b ist ausschließlich auf Branch main zulässig")
    require(
        git_is_ancestor(V2737B_GATE_BOOTSTRAP_BASE_SHA, head),
        "v27.37b-Bootstrap-Basis ist kein Vorfahr von HEAD",
    )
    require(
        git_is_ancestor(V2737B_GATE_BOOTSTRAP_BASE_SHA, origin_main)
        and git_is_ancestor(origin_main, head),
        "origin/main liegt außerhalb der linearen v27.37b-Grenze",
    )
    diff_files = frozenset(
        line.strip().replace("\\", "/")
        for line in run_git(["diff", "--name-only"]).splitlines()
    ...
    if not roles and head == V2737B_GATE_BOOTSTRAP_BASE_SHA:
    ...
    elif roles == ("v2737b_bootstrap",):
        if task_state == "v2737a_closed" and not working_files:
            phase = "v2737b_gate_bootstrap_committed"
        elif task_state == "v2737a_closed" and working_files:
            phase = "v2737b_gate_bootstrap_repair_prepared"
        else:
            raise ValidationError("Zustand nach v27.37b-Bootstrap ist unzulässig")
    elif roles == ("v2737b_bootstrap", "v2737b_bootstrap_repair"):
        if task_state == "v2737a_closed" and not working_files:
            phase = "v2737b_gate_bootstrap_repair_committed"
        elif task_state == "v2737b_authorized" and working_files:
            phase = "v2737b_authorization_prepared"
        else:
            raise ValidationError("Zustand nach v27.37b-GATE-BOOTSTRAP-REPAIR ist unzulässig")
    elif roles == (
        "v2737b_bootstrap",
        "v2737b_bootstrap_repair",
        "v2737b_gate",
        ...
        phase = (
            "v2737b_authorization_committed"
            if not working_files
            else "v2737b_implementation_prepared"
        )
    elif roles == (
        "v2737b_bootstrap",
        "v2737b_bootstrap_repair",
        ...
        phase = "v2737b_closure_committed"
    else:
        raise ValidationError("v27.37b-Historie ist unzulässig")
    if phase in {
        "v2737b_gate_bootstrap_repair_prepared",
        "v2737b_gate_bootstrap_repair_committed",
        "v2737b_authorization_prepared",
    ...
    require(
        ...
        "Aktueller Zustand liegt außerhalb der engen v27.37b-Task-/Scope-/Phasen-Allowlist",
    )
    return phase


def run_v2737b_gate_bootstrap_manipulation_matrix(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
    ...
    for phase, task_state, working_files, history_roles in positive_cases:
        require(
            v2737b_scope_facts_are_valid(
                phase=phase,
                task_state=task_state,
            ...
            f"v27.37b-Positivsimulation fehlgeschlagen: {phase}",
        )
    ...
    negative_cases = (
        ("unknown_future_task", "v2737b_authorized", frozenset(), repair_history),
        ...
        ("v2737b_gate_bootstrap_committed", "v2737a_closed", frozenset(), ("v2737b_bootstrap", "v2737b_bootstrap")),
        ...
        ("v2737b_gate_bootstrap_repair_committed", "v2737a_closed", frozenset(), (*repair_history, "v2737b_bootstrap_repair")),
        ("v2737b_authorization_prepared", "v2737b_authorized", frozenset({"docs/tasks/CURRENT_TASK.md"}), ("v2737b_bootstrap",)),
        ("v2737b_authorization_prepared", "v2737b_authorized", frozenset(), repair_history),
        ("v2737b_authorization_prepared", "v2737b_authorized", frozenset({"app.js"}), repair_history),
        ("v2737b_authorization_prepared", "invalid", frozenset({"docs/tasks/CURRENT_TASK.md"}), repair_history),
        ("v2737b_authorization_committed", "v2737b_authorized", frozenset(), (*authorization_history, "v2737b_gate")),
        ("v2737b_implementation_prepared", "v2737b_authorized", V2737B_IMPLEMENTATION_FILES | {"index.html"}, authorization_history),
        ("v2737b_implementation_prepared", "v2737b_authorized", V2737B_IMPLEMENTATION_FILES - {"tools/preflight.py"}, authorization_history),
        ("v2737b_implementation_committed", "v2737b_authorized", frozenset(), authorization_history),
        ...
        ("v2737b_closure_committed", "v2737b_closed", frozenset(), implementation_history),
    )
    for phase, task_state, working_files, history_roles in negative_cases:
        require(
            not v2737b_scope_facts_are_valid(
                phase=phase,
                task_state=task_state,
                working_files=frozenset(working_files),
                history_roles=history_roles,
                products_unchanged=True,
            ),
            f"v27.37b-Scope-Manipulation wurde nicht blockiert: {phase}",
        )
    require(
        not v2737b_scope_facts_are_valid(
            phase="v2737b_gate_bootstrap_prepared",
            task_state="v2737a_closed",
            working_files=V2737B_BOOTSTRAP_FILES,
            history_roles=(),
            products_unchanged=False,
        ),
        "v27.37b-Frozen-Produktmanipulation wurde nicht blockiert",
    )
    manipulation_checks = len(negative_cases) + 1
    document_sections = (
        (
            extract_v2737b_bootstrap_section(state_text, "PROJECT_STATE_CURRENT"),
            "PROJECT_STATE_CURRENT",
        ),
        (
            extract_v2737b_bootstrap_section(task_text, "CURRENT_TASK"),
            "CURRENT_TASK",
        ),
        (
            extract_v2737b_bootstrap_section(cursor_text, "CURSOR_MASTER_CONTEXT_ACCAOUI"),
            "CURSOR_MASTER_CONTEXT_ACCAOUI",
        ),
        (
            extract_v2737b_bootstrap_section(masterlist_text, "PROJECT_MASTERLIST"),
            "PROJECT_MASTERLIST",
        ),
    )
    for section, name in document_sections:
        for marker in V2737B_BOOTSTRAP_REQUIRED_MARKERS:
            require(
                section.count(marker) == 1,
                f"{name}: v27.37b-Manipulation benötigt Pflichtmarker: {marker}",
            )
            mutated = replace_v2737b_exact_once(
                section, marker, ""
            )
            try:
                validate_v2737b_bootstrap_section(mutated, name)
            except ValidationError:
                manipulation_checks += 1
                continue
            raise ValidationError(
                f"v27.37b-Dokumentmanipulation wurde nicht blockiert: {name} / {marker}"
            )
    canonical_section, canonical_name = document_sections[0]
    bootstrap_start = "Der einmalige atomare Bootstrap umfasst exakt:"
    bootstrap_end = "Keine siebte Datei und keine Produktdatei sind zulässig."
    implementation_start = "Der spätere Implementierungsscope umfasst exakt:"
    implementation_end = "Die spätere Factory ist"
    for path in V2737B_BOOTSTRAP_FILE_ORDER:
        ...
        try:
            validate_v2737b_bootstrap_section(mutated, canonical_name)
        except ValidationError:
            manipulation_checks += 1
            continue
        raise ValidationError(
            f"v27.37b-Bootstrap-Dateimanipulation wurde nicht blockiert: {path}"
        )
    for path in V2737B_IMPLEMENTATION_FILE_ORDER:
        needle = f"- `{path}`\n"
        mutated = replace_v2737b_in_delimited_block(
        ...
        try:
            ...
            continue
        raise ValidationError(
            f"v27.37b-Implementierungsdateimanipulation wurde nicht blockiert: {path}"
        )
    added_file_anchor = f"- `{V2737B_IMPLEMENTATION_FILE_ORDER[0]}`\n"
    added_file_addition = "- `app.js`\n"
    added_implementation_file = insert_v2737b_after_in_delimited_block(
        canonical_section,
        implementation_start,
        implementation_end,
        added_file_anchor,
    ...
    require(
        added_implementation_file.count(added_file_anchor) == 1
        and added_implementation_file.count(added_file_addition) == 1,
        "v27.37b-zusätzliche Implementierungsdatei war nicht rein additiv",
    )
    try:
        validate_v2737b_bootstrap_section(
            added_implementation_file, canonical_name
        )
    except ValidationError:
        manipulation_checks += 1
    else:
        raise ValidationError(
            "v27.37b-zusätzliche Implementierungsdatei wurde nicht blockiert"
        )
    for label, needle, replacement in V2737B_CANONICAL_CONTRACT_MUTATIONS:
        ...
        try:
            ...
            continue
        raise ValidationError(
            f"v27.37b-Vertragsmanipulation wurde nicht blockiert: {label}"
        )
    for label, anchor, addition in V2737B_CANONICAL_ADDITIVE_MUTATIONS:
        mutated = insert_v2737b_after_exact_once(
            canonical_section, anchor, addition
        )
        require(
            mutated.count(anchor) == 1 and mutated.count(addition) == 1,
            f"v27.37b-additive Vertragsmutation war nicht rein additiv: {label}",
        )
        try:
            validate_v2737b_bootstrap_section(mutated, canonical_name)
        except ValidationError:
            ...
            continue
        raise ValidationError(
            f"v27.37b-additive Vertragsmanipulation wurde nicht blockiert: {label}"
        )
    future_sha_anchor = "Keine echten Teilnehmerdaten."
    future_sha_addition = "\nZukünftige SHA: `" + ("a" * 40) + "`"
    future_sha_section = insert_v2737b_after_exact_once(
        canonical_section, future_sha_anchor, future_sha_addition
    )
    try:
        validate_v2737b_bootstrap_section(future_sha_section, canonical_name)
    except ValidationError:
        manipulation_checks += 1
    else:
        raise ValidationError(
            "v27.37b-zukünftige-SHA-Manipulation wurde nicht blockiert"
        )
    repair_document_sections = (
        (
            extract_v2737b_gate_bootstrap_repair_section(
                state_text, "PROJECT_STATE_CURRENT"
            ),
            "PROJECT_STATE_CURRENT",
        ),
        (
            extract_v2737b_gate_bootstrap_repair_section(task_text, "CURRENT_TASK"),
            "CURRENT_TASK",
        ),
        (
            extract_v2737b_gate_bootstrap_repair_section(
    ...
    for section, name in repair_document_sections:
        for marker in V2737B_GATE_BOOTSTRAP_REPAIR_REQUIRED_MARKERS:
            require(
                section.count(marker) == 1,
                f"{name}: v27.37b-Repair-Manipulation benötigt Pflichtmarker: {marker}",
            )
            mutated = replace_v2737b_exact_once(section, marker, "")
            try:
                validate_v2737b_gate_bootstrap_repair_section(mutated, name)
            except ValidationError:
                manipulation_checks += 1
                continue
            raise ValidationError(
                f"v27.37b-Repair-Dokumentmanipulation wurde nicht blockiert: {name} / {marker}"
            )
    repair_section, repair_name = repair_document_sections[0]
    ...
    for path in V2737B_BOOTSTRAP_FILE_ORDER:
        ...
        try:
            validate_v2737b_gate_bootstrap_repair_section(mutated, repair_name)
        except ValidationError:
            manipulation_checks += 1
            continue
        raise ValidationError(
            f"v27.37b-Repair-Dateimanipulation wurde nicht blockiert: {path}"
        )
    repair_with_product_file = insert_v2737b_after_in_delimited_block(
        repair_section,
        repair_file_list_start,
        repair_file_list_end,
    ...
    try:
        validate_v2737b_gate_bootstrap_repair_section(
            repair_with_product_file, repair_name
        )
    except ValidationError:
        manipulation_checks += 1
    else:
        raise ValidationError(
            "v27.37b-Repair-Produktdateimanipulation wurde nicht blockiert"
        )
    repair_with_future_sha = insert_v2737b_after_exact_once(
        repair_section,
        "Keine echten Teilnehmerdaten.",
    ...
    try:
        validate_v2737b_gate_bootstrap_repair_section(
            repair_with_future_sha, repair_name
        )
    except ValidationError:
        manipulation_checks += 1
    else:
    ...
    for phase in V2737B_PHASES:
        phase_fields = v2737b_expected_current_task_fields_for_phase(phase)
        expected_state = expected_state_by_fields[tuple(phase_fields.items())]
        require(
            synthetic_v2737b_current_task_state(
                build_v2737b_synthetic_current_task(phase_fields)
            )
            == expected_state,
            f"v27.37b-Phase nutzt nicht den kanonischen CURRENT_TASK-Vertrag: {phase}",
        )
    try:
        v2737b_expected_current_task_fields_for_phase("unknown_future_task")
    except ValidationError:
        manipulation_checks += 1
    else:
        raise ValidationError(
            "Unbekannte v27.37b-Phase wurde vom CURRENT_TASK-Vertrag akzeptiert"
        )
    ...
    for baseline_label, fields, expected_state in expected_fields_by_state:
        baseline_task = build_v2737b_synthetic_current_task(dict(fields))
        require(
            v2737b_current_task_state(baseline_task) == expected_state,
            f"v27.37b-CURRENT_TASK-Positivvertrag fehlt: {baseline_label}",
        )
        for field, replacement_value in invalid_field_values.items():
            ...
            require(
                synthetic_v2737b_current_task_state(mutated) == "invalid",
                f"v27.37b-CURRENT_TASK-Feldmanipulation wurde nicht blockiert: {baseline_label} / {field}",
            )
            manipulation_checks += 1
        missing_field = replace_v2737b_exact_once(
            baseline_task,
            f"Commit erlaubt: {fields['Commit erlaubt']}\n",
            "",
        )
        duplicate_field = duplicate_v2737b_exact_once(
            baseline_task,
            f"Push erlaubt: {fields['Push erlaubt']}",
        )
        unknown_field = insert_v2737b_after_exact_once(
            baseline_task,
            f"Push erlaubt: {fields['Push erlaubt']}",
            "\nUnbekanntes Steuerfeld: verboten",
        )
        missing_main_header = replace_v2737b_exact_once(
            baseline_task,
            V2737A_CURRENT_TASK_HEADER_MARKER,
            "# Manipulierter aktueller Task",
        )
        duplicate_main_header = duplicate_v2737b_exact_once(
            baseline_task,
            V2737A_CURRENT_TASK_HEADER_MARKER,
        )
        missing_first_section = replace_v2737b_exact_once(
            baseline_task,
            "## Synthetischer Folgeabschnitt",
            "Synthetischer Folgeabschnitt",
        )
        for label, mutated in (
            ...
            require(
                synthetic_v2737b_current_task_state(mutated) == "invalid",
                f"v27.37b-CURRENT_TASK-Strukturmanipulation wurde nicht blockiert: {baseline_label} / {label}",
            )
            manipulation_checks += 1
        ...
        with_historical_fields = baseline_task + historical_block
        require(
            v2737b_current_task_state(with_historical_fields) == expected_state,
            f"Historische Felder beschädigen den gültigen CURRENT_TASK-Kopf: {baseline_label}",
        )
        manipulation_checks += 1
        invalid_current = replace_v2737b_exact_once(
            baseline_task,
            f"Status: {fields['Status']}",
            "Status: INVALID_STATUS",
        )
        require(
            synthetic_v2737b_current_task_state(invalid_current + historical_block)
            == "invalid",
            f"Historische Felder retten einen ungültigen CURRENT_TASK-Kopf: {baseline_label}",
        )
        manipulation_checks += 1
    destroyed_bootstrap_document = replace_v2737b_exact_once(
        state_text,
        V2737B_BOOTSTRAP_SECTION_HEADING,
    ...
    for label, mutated_document in (
        ...
        ("zerstörter Implementierungsabschnitt", destroyed_implementation_document),
    ):
        require(
            mutated_document != state_text,
            f"v27.37b-Dokumentstruktur-Manipulation war wirkungslos: {label}",
        )
        require(
            not synthetic_v2737b_bootstrap_document_is_valid(
                mutated_document, "PROJECT_STATE_CURRENT"
            ),
            f"v27.37b-Dokumentstruktur-Manipulation wurde nicht blockiert: {label}",
        )
        manipulation_checks += 1
    return manipulation_checks, len(positive_cases), len(negative_cases) + 1
...
V2737C_BOOTSTRAP_FILES = frozenset(V2737C_BOOTSTRAP_FILE_ORDER)
...
V2737C_REQUIRED_MARKERS = (
    ...
    "Die CommonJS-Oberflächen und das bestehende Auth-/Session-Verhalten bleiben fachlich unverändert.",
    ...
    "keine Auth-Operation, keinen Clientzugriff und keine Sessionauflösung ausführen",
    "`initializeClient()`",
    "`createClient()`",
    "`getState()`",
    "`index.html`",
...
def extract_v2737c_bootstrap_section(text: str, document_name: str) -> str:
    require(
        text.count(V2737C_BOOTSTRAP_SECTION_HEADING) == 1,
        f"{document_name}: v27.37c-GATE-BOOTSTRAP fehlt oder ist doppelt",
    )
    tail = text.split(V2737C_BOOTSTRAP_SECTION_HEADING, 1)[1]
    next_heading = re.search(r"(?m)^## ", tail)
    return tail[:next_heading.start()] if next_heading else tail


def validate_v2737c_bootstrap_section(
    section: str, document_name: str
) -> None:
    for marker in V2737C_REQUIRED_MARKERS:
        require(
            marker in section,
            f"{document_name}: v27.37c-Pflichtaussage fehlt: {marker}",
        )
    ...
    require(
        ...
        f"{document_name}: v27.37c-Bootstrap-Dateiliste nicht eindeutig",
    )
    bootstrap_list = section.split(bootstrap_start, 1)[1].split(
        bootstrap_end, 1
    )[0].strip()
    require(
        bootstrap_list
        == "\n".join(f"- `{path}`" for path in V2737C_BOOTSTRAP_FILE_ORDER),
        f"{document_name}: v27.37c-Bootstrap muss exakt sechs Dateien enthalten",
    )
    ...
    require(
        section.count(implementation_start) == 1
        and section.count(implementation_end) == 1,
        f"{document_name}: v27.37c-Implementierungsscope nicht eindeutig",
    )
    implementation_list = section.split(implementation_start, 1)[1].split(
        implementation_end, 1
    )[0].strip()
    require(
        ...
        f"{document_name}: v27.37c-Implementierungsscope ist nicht exakt",
    )

    browser_start = "Die vorgesehenen Browser-Grenzen heißen exakt:"
    browser_end = "Ein Browser-Export darf"
    require(
        section.count(browser_start) == 1
        and section.count(browser_end) == 1,
        f"{document_name}: v27.37c-Browser-Grenzen nicht eindeutig",
    )
    browser_list = section.split(browser_start, 1)[1].split(
        browser_end, 1
    )[0].strip()
    expected_browser_list = "\n".join((
        "- `window.ACCAOUI_PARTICIPANT_AUTH_SESSION_ADAPTER_FACTORY`",
        "- `window.ACCAOUI_PARTICIPANT_AUTH_SESSION_BOOTSTRAP_BRIDGE_FACTORY`",
    ))
    require(
        browser_list == expected_browser_list,
        f"{document_name}: v27.37c-Browser-Grenzen sind nicht exakt",
    )

    shas = frozenset(re.findall(r"\b[0-9a-f]{40}\b", section))
    require(
        shas == frozenset({V2737C_GATE_BOOTSTRAP_BASE_SHA}),
        f"{document_name}: v27.37c-Basis fehlt oder zukünftige SHA hartcodiert",
    )


def validate_v2737c_bootstrap_documents(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
    ...
    for document, name in (
        ...
        validate_v2737c_bootstrap_section(
            extract_v2737c_bootstrap_section(document, name),
            name,
        )


def validate_v2737b_closed_at_v2737c_base() -> str:
    paths = (
        "docs/PROJECT_STATE_CURRENT.md",
        "docs/tasks/CURRENT_TASK.md",
        "docs/CURSOR_MASTER_CONTEXT_ACCAOUI.md",
    ...
    documents = tuple(
        read_v2735f_commit_document(
            V2737C_GATE_BOOTSTRAP_BASE_SHA, path
        )
        for path in paths
    )

    roles, implementation_commit = read_v2737b_history(
    ...
    require(
        ...
        "v27.37b ist an der v27.37c-Basis nicht vollständig abgeschlossen",
    )
    require(
        implementation_commit is not None,
        "v27.37b-Implementierungscommit an v27.37c-Basis fehlt",
    )
    ...
    validate_v2737b_authorization_documents(*documents)
    validate_v2737b_completion_documents(
        *documents, implementation_commit
    )
    validate_v2737b_frozen_implementation(
        implementation_commit,
        revision=V2737C_GATE_BOOTSTRAP_BASE_SHA,
    )
    require(
        v2737b_current_task_state(documents[1]) == "v2737b_closed",
        "v27.37b-CURRENT_TASK ist an der v27.37c-Basis nicht geschlossen",
    )
    return implementation_commit


def read_v2737c_gate_bootstrap_history(
    head: str,
) -> tuple[str, ...]:
    commits = [
        line.strip()
    ...
    for commit in commits:
        ...
        if (
            ...
            and v2737b_current_task_state(task_text) == "v2737b_closed"
        ):
            documents = tuple(
                read_v2735f_commit_document(commit, path)
                for path in (
...
def v2737c_scope_facts_are_valid(
    *,
    phase: str,
    task_state: str,
    working_files: frozenset[str],
...
def validate_v2737c_gate_bootstrap_lifecycle(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
) -> tuple[str, str]:
    implementation_commit = validate_v2737b_closed_at_v2737c_base()
    ...
    task_state = v2737b_current_task_state(task_text)
    require(
        task_state == "v2737b_closed",
        "v27.37c-GATE-BOOTSTRAP darf CURRENT_TASK nicht autorisieren",
    )
    ...
    require(
        ...
        "v27.37c-Bootstrap-Basis ist kein Vorfahr von HEAD",
    )
    require(
    ...
    require(
        not staged_files,
        "v27.37c-GATE-BOOTSTRAP darf keine staged Dateien enthalten",
    )

    working_files = diff_files | untracked_files
    roles = read_v2737c_gate_bootstrap_history(head)

    if (
        not roles
        and head == V2737C_GATE_BOOTSTRAP_BASE_SHA
        and working_files == V2737C_BOOTSTRAP_FILES
    ):
        ...
        raise ValidationError(
            "Aktueller Zustand liegt außerhalb der engen "
            "v27.37c-GATE-BOOTSTRAP-Allowlist"
        )

    require(
        v2737c_scope_facts_are_valid(
            phase=phase,
            task_state=task_state,
            working_files=working_files,
            history_roles=roles,
    ...
    return phase, implementation_commit


def run_v2737c_gate_bootstrap_manipulation_matrix(
    state_text: str,
    task_text: str,
    cursor_text: str,
    masterlist_text: str,
    ...
    for phase, task_state, files, roles in positive_cases:
        require(
            v2737c_scope_facts_are_valid(
                phase=phase,
                task_state=task_state,
    ...
    for phase, task_state, files, roles in negative_cases:
        require(
            not v2737c_scope_facts_are_valid(
                phase=phase,
                task_state=task_state,
                working_files=files,
                history_roles=roles,
            ),
            f"v27.37c-Scope-Manipulation nicht blockiert: {phase}",
        )
    ...
    for document, name in documents:
        section = extract_v2737c_bootstrap_section(document, name)
        for marker in (
            "v27.37c-GATE-BOOTSTRAP ist ausschließlich Kontrollinfrastruktur.",
            "Supabase bleibt NICHT LIVE.",
        ):
            require(
                section.count(marker) == 1,
                f"{name}: v27.37c-Manipulationsmarker nicht eindeutig",
            )
            mutated = section.replace(marker, "", 1)
            try:
                validate_v2737c_bootstrap_section(mutated, name)
            except ValidationError:
                document_checks += 1
            else:
                raise ValidationError(
                    f"{name}: v27.37c-Dokumentmanipulation nicht blockiert"
                )

    return (
        document_checks + len(negative_cases),
        len(positive_cases),
        len(negative_cases),
    )
...
def detect_v2737e_phase(
    expected_working_files: frozenset[str] | None = None,
    *, _closed_snapshot: str | None = None,
) -> str | None:
    require(_closed_snapshot in (None, V2737F_BASE_SHA),
            "v27.37e: nur vollständiger historischer Abschluss als Snapshot erlaubt")
    if _closed_snapshot is None:
        successor = detect_v2737f_phase(expected_working_files)
    ...
    current.update(
        branch="main" if _closed_snapshot else run_git(["branch", "--show-current"]).strip(),
        head=head, origin=head if _closed_snapshot else run_git(["rev-parse", "origin/main"]).strip(),
        staged=frozenset() if _closed_snapshot else paths(["diff", "--cached", "--name-only"]), working=working,
    )
    ...
    for path in (
        ...
        require(content(_closed_snapshot, path) == content(V2737E_BASE_SHA, path),
...
# Strict successor of the complete v27.37e closure.
V2737F_BASE_SHA = "514b0c25805a53f711d963f2fe3b7c08d4eef3e1"
V2737F_TITLE = "v27.37f – Auth-/Session-Anbindung an den App-Start"
V2737F_AUTHORIZATION_HEADING = "## Autorisierter Task v27.37f"
V2737F_DOCUMENT_PATHS = V2737D_DOCUMENT_PATHS
V2737F_GATE_FILES = frozenset(V2737D_GATE_FILE_ORDER)
V2737F_IMPLEMENTATION_FILE_ORDER = (
    "app.js",
    "tools/check-participant-auth-session-app-entry-v2737f.py",
    "docs/PARTICIPANT_AUTH_SESSION_APP_ENTRY_V2737F.md",
    "tools/preflight.py",
)
V2737F_IMPLEMENTATION_FILES = frozenset(V2737F_IMPLEMENTATION_FILE_ORDER)
V2737F_CLOSURE_FILES = frozenset(V2737F_DOCUMENT_PATHS)
V2737F_AUTHORIZED_TASK_FIELDS = {
    **V2737E_CLOSED_TASK_FIELDS,
    "Task-ID": "v27.37f", "Status": "AUTHORIZED", "Autorisiert": "JA",
    "Titel": V2737F_TITLE,
    "Erlaubte Implementierungsdateien": ", ".join(
        f"`{path}`" for path in V2737F_IMPLEMENTATION_FILE_ORDER
    ),
}
V2737F_CLOSED_TASK_FIELDS = {
    **V2737E_CLOSED_TASK_FIELDS,
...
V2737F_APP_LINE = '<script src="app.js?v=24.8"></script>\n'
V2737F_CHECKER_REGISTRATION = "V2737F_IMPLEMENTATION_CHECKER = None\n"
V2737F_REGISTERED_CHECKER = (
    'V2737F_IMPLEMENTATION_CHECKER = '
    '"tools/check-participant-auth-session-app-entry-v2737f.py"\n'
)

V2737F_PHASES = frozenset("v2737f_" + name for name in (
    "authorization_prepared", "authorization_committed", "implementation_prepared",
    "implementation_committed", "closure_prepared", "closure_committed",
))
V2737F_FROZEN_REGRESSION_PATHS = (
    "index.html",
    "data/supabase-participant-access-adapter.js",
    "data/supabase-participant-access-bootstrap-bridge.js",
    "data/supabase-participant-access-browser-provider.js",
...
def v2737f_implementation_preflight(gate_preflight: str) -> str:
    require(gate_preflight.count(V2737F_CHECKER_REGISTRATION) == 1
            and V2737F_REGISTERED_CHECKER not in gate_preflight,
            "v27.37f: uneindeutige Loader-Checker-Registrierung im Gate")
    return gate_preflight.replace(V2737F_CHECKER_REGISTRATION, V2737F_REGISTERED_CHECKER)


def v2737f_classify_phase(
    roles: tuple[str, ...], working: frozenset[str], task_fields: dict[str, str],
) -> str:
    cases = (
        ((), V2737F_GATE_FILES, V2737F_AUTHORIZED_TASK_FIELDS, "authorization_prepared"),
        (("gate",), frozenset(), V2737F_AUTHORIZED_TASK_FIELDS, "authorization_committed"),
        (("gate",), V2737F_IMPLEMENTATION_FILES, V2737F_AUTHORIZED_TASK_FIELDS,
         "implementation_prepared"),
        (("gate", "implementation"), frozenset(), V2737F_AUTHORIZED_TASK_FIELDS,
         "implementation_committed"),
        (("gate", "implementation"), V2737F_CLOSURE_FILES, V2737F_CLOSED_TASK_FIELDS,
         "closure_prepared"),
        (("gate", "implementation", "closure"), frozenset(), V2737F_CLOSED_TASK_FIELDS,
         "closure_committed"),
    )
    matches = [phase for history, scope, fields, phase in cases
               if roles == history and working == scope and task_fields == fields]
    require(len(matches) == 1, "v27.37f: unzulässiger Task-, Scope- oder Phasenübergang")
    return "v2737f_" + matches[0]


def v2737f_expected_index(base_index: str) -> str:
    return base_index


def validate_v2737f_facts(
    base_documents: tuple[str, ...], base_index: str,
    commits: tuple[dict, ...], current: dict, base_app: str,
) -> str:
    """Pure lifecycle validator, shared by real Git reads and isolated tests.
    ...
    """
    require(current["branch"] == "main" and not current["staged"],
            "v27.37f: main und leerer Index erforderlich")
    require(len(commits) <= 3, "v27.37f: unbekannter oder wiederholter Folgecommit")
    authorization = v2737f_authorization_documents(base_documents)
    parent = V2737F_BASE_SHA
    gate_preflight = None
    gate_checker = None
    roles = []
    scopes = (V2737F_GATE_FILES, V2737F_IMPLEMENTATION_FILES, V2737F_CLOSURE_FILES)
    for number, commit in enumerate(commits):
        require(commit["parents"] == (parent,),
                "v27.37f: Commit muss direkt und linear folgen")
        require(commit["paths"] == scopes[number],
                "v27.37f: Commit-Dateiumfang oder Übergangsreihenfolge verletzt")
        require(commit["sha"] not in {V2737F_BASE_SHA, *(c["sha"] for c in commits[:number])},
                "v27.37f: wiederholte Commitidentität")
        expected_documents = (authorization if number < 2 else
                              v2737f_completion_documents(authorization, commits[1]["sha"]))
        require(commit["documents"] == expected_documents,
                "v27.37f: Commit-Dokumentvertrag oder historischer Verlauf verletzt")
        require(commit["index"] == (base_index if number == 0
                                   else v2737f_expected_index(base_index)),
                "v27.37f: Index gegenüber Basis verändert")
        v2737f_validate_product_snapshot(commit, base_app, number >= 1)
        if number == 2:
            require(commit["app"] == commits[1]["app"]
                    and commit["artifacts"] == commits[1]["artifacts"],
                    "v27.37f: Implementierungsdateien in Closure verändert")
        if number == 0:
            gate_preflight = commit["preflight"]
            gate_checker = commit["checker"]
            v2737f_implementation_preflight(gate_preflight)
        else:
            require(commit["preflight"] == v2737f_implementation_preflight(gate_preflight),
                    "v27.37f: andere Preflight-Änderung als Checker-Registrierung")
            require(commit["checker"] == gate_checker,
                    "v27.37f: Continuity-Checker nach Gate verändert")
        roles.append(("gate", "implementation", "closure")[number])
        parent = commit["sha"]
    require(current["head"] == parent, "v27.37f: HEAD passt nicht zur linearen Historie")
    require(current["origin"] in {V2737F_BASE_SHA, *(c["sha"] for c in commits)},
            "v27.37f: origin/main ist kein legitimer Lifecycle-Vorfahr")
    phase = v2737f_classify_phase(
        tuple(roles), current["working"],
        v2737a_current_task_header_fields(current["documents"][1]),
    )
    closed = phase in ("v2737f_closure_prepared", "v2737f_closure_committed")
    implemented = len(commits) >= 2 or phase == "v2737f_implementation_prepared"
    require(current["documents"] == (
        v2737f_completion_documents(authorization, commits[1]["sha"]) if closed
        else authorization), "v27.37f: aktuelle Dokumente oder Historie verändert")
    require(current["index"] == (v2737f_expected_index(base_index) if implemented
                                 else base_index),
            "v27.37f: Index gegenüber Basis verändert")
    if gate_preflight is None:
        v2737f_implementation_preflight(current["preflight"])
    else:
        require(current["preflight"] == (
            v2737f_implementation_preflight(gate_preflight) if implemented
            else gate_preflight), "v27.37f: Preflight-Registrierung oder Regression verändert")
        require(current["checker"] == gate_checker,
                "v27.37f: bestehende Lifecycle-Kontrolle nach Gate verändert")
    v2737f_validate_product_snapshot(current, base_app, implemented)
    if len(commits) >= 2:
        require(current["app"] == commits[1]["app"]
                and current["artifacts"] == commits[1]["artifacts"],
                "v27.37f: Implementierungsdateien nach Implementation verändert")
    return phase



def v2737f_app_remainder(source: str) -> str:
    """Keep every app byte outside the two startup functions and one helper block."""
    begin = "// BEGIN v27.37f auth/session app entry\n"
    end = "// END v27.37f auth/session app entry\n"
    if begin in source or end in source:
        require(source.count(begin) == source.count(end) == 1,
                "v27.37f: uneindeutiger App-Hilfsblock")
        left, tail = source.split(begin)
        block, right = tail.split(end)
        require(right.startswith("async function initAppBoot() {"),
                "v27.37f: Hilfsblock nicht unmittelbar vor initAppBoot")
        source = left + right
    for start, following in (
        ("async function initAppBoot() {", "async function loadOptionalSupabaseConfig() {"),
        ("async function initAuthFlow() {",
         "function isParticipantAccessBrowserLoaderRequestedV2736F() {"),
    ):
        require(source.count(start) == 1 and source.count(following) == 1,
                "v27.37f: App-Funktionsgrenzen uneindeutig")
        left, tail = source.split(start)
        body, right = tail.split(following)
        require(body.rstrip().endswith("}"), "v27.37f: App-Funktionsende fehlt")
        source = left + start + "\n}\n\n" + following + right
    return source


def v2737f_validate_product_snapshot(snapshot: dict, base_app: str, implemented: bool) -> None:
    if not implemented:
        require(snapshot["app"] == base_app and snapshot["artifacts"] == (None, None),
                "v27.37f: Produktimplementation vor Gate-Abschluss")
        return
    require(snapshot["app"] != base_app
            and v2737f_app_remainder(snapshot["app"]) == v2737f_app_remainder(base_app),
            "v27.37f: App-Änderung außerhalb autorisierter Startgrenzen")
    require(len(snapshot["artifacts"]) == 2
            ...
            "v27.37f: Implementierungschecker oder Dokument fehlt")


def detect_v2737f_phase(
    expected_working_files: frozenset[str] | None = None,
    *, _closed_snapshot: str | None = None,
) -> str | None:
    require(_closed_snapshot in (None, V2737G_BASE_SHA),
            "v27.37f: nur vollständiger historischer Abschluss als Snapshot erlaubt")
    if _closed_snapshot is None:
        successor = detect_v2737g_phase(expected_working_files)
        if successor is not None:
            return successor
    head = _closed_snapshot or run_git(["rev-parse", "HEAD"]).strip()
    state = (read_required_text(STATE_PATH) if _closed_snapshot is None
             else read_v2735f_commit_document(_closed_snapshot, "docs/PROJECT_STATE_CURRENT.md"))
    active = V2737F_AUTHORIZATION_HEADING in state
    if not git_is_ancestor(V2737F_BASE_SHA, head):
        require(not active, "v27.37f: falsche Git-Basis")
        return None
    if not active and head == V2737F_BASE_SHA:
        return None
    require(active, "v27.37f: unbekannter Nachfolgetask")
    require(detect_v2737e_phase(_closed_snapshot=V2737F_BASE_SHA)
            == "v2737e_closure_committed", "v27.37f: v27.37e nicht vollständig geschlossen")
    ...
    def snapshot(ref):
        ...
        artifacts = tuple(
            ...
            for path in V2737F_IMPLEMENTATION_FILE_ORDER[1:3]
        )
        return {
            "documents": tuple(content(ref, p) for p in V2737F_DOCUMENT_PATHS),
            "index": content(ref, "index.html"),
            "app": content(ref, "app.js"), "artifacts": artifacts,
            "preflight": content(ref, "tools/preflight.py"),
            "checker": content(ref, CHECKER_RELATIVE_PATH),
        }

    refs = run_git(["rev-list", "--reverse", V2737F_BASE_SHA + ".." + head]).splitlines()
    require(len(refs) <= 3, "v27.37f: mehr als Gate, Implementation und Closure")
    commits = []
    previous = V2737F_BASE_SHA
    for ref in refs:
        commit = snapshot(ref)
        commit.update(
            sha=ref, parents=tuple(run_git(["rev-list", "--parents", "-n", "1", ref]).split()[1:]),
    ...
    require(expected_working_files is None or working == expected_working_files,
            "v27.37f: Working-Tree-Erfassung widersprüchlich")
    current = snapshot(_closed_snapshot)
    current.update(
        branch="main" if _closed_snapshot else run_git(["branch", "--show-current"]).strip(),
        head=head, origin=head if _closed_snapshot else run_git(["rev-parse", "origin/main"]).strip(),
    ...
    phase = validate_v2737f_facts(
        tuple(content(V2737F_BASE_SHA, p) for p in V2737F_DOCUMENT_PATHS),
        content(V2737F_BASE_SHA, "index.html"), tuple(commits), current,
        content(V2737F_BASE_SHA, "app.js"),
    )
    for path in V2737F_FROZEN_REGRESSION_PATHS:
        require(content(_closed_snapshot, path) == content(V2737F_BASE_SHA, path),
                f"v27.37f: eingefrorene Regression verändert: {path}")
    return phase

def run_v2737f_lifecycle_self_checks() -> tuple[int, int]:
    """Exercise transitions and document/index contracts entirely in memory."""
    import copy
    base = tuple(read_v2735f_commit_document(V2737F_BASE_SHA, p)
                 for p in V2737F_DOCUMENT_PATHS)
    index = read_v2735f_commit_document(V2737F_BASE_SHA, "index.html")
    base_app = read_v2735f_commit_document(V2737F_BASE_SHA, "app.js")
    app_fixture = base_app.replace("async function initAppBoot() {",
        "// BEGIN v27.37f auth/session app entry\n"
        "// isolated fixture, no product implementation\n"
        "// END v27.37f auth/session app entry\nasync function initAppBoot() {")
    auth = v2737f_authorization_documents(base)
    identities = [hashlib.sha1(("isolated-v2737f-" + role).encode()).hexdigest()
                  for role in ("gate", "implementation", "closure")]
    preflight = V2737F_CHECKER_REGISTRATION + "# existing regression checks\n"
    checker = "# unchanged lifecycle checker\n"
    gate = dict(sha=identities[0], parents=(V2737F_BASE_SHA,),
                paths=V2737F_GATE_FILES, documents=auth, index=index,
                preflight=preflight, checker=checker, app=base_app, artifacts=(None, None))
    implementation = dict(
        sha=identities[1], parents=(identities[0],), paths=V2737F_IMPLEMENTATION_FILES,
        documents=auth, index=v2737f_expected_index(index),
        preflight=v2737f_implementation_preflight(preflight), checker=checker,
        app=app_fixture, artifacts=("# isolated checker\n", "# isolated document\n"),
    )
    closure = dict(
        sha=identities[2], parents=(identities[1],), paths=V2737F_CLOSURE_FILES,
        documents=v2737f_completion_documents(auth, identities[1]),
        index=implementation["index"], preflight=implementation["preflight"], checker=checker,
        app=app_fixture, artifacts=implementation["artifacts"],
    )
    states = []
    for commits, scope, source in (
        ((), V2737F_GATE_FILES, gate),
        ((gate,), frozenset(), gate),
        ((gate,), V2737F_IMPLEMENTATION_FILES, implementation),
        ((gate, implementation), frozenset(), implementation),
        ((gate, implementation), V2737F_CLOSURE_FILES, closure),
        ((gate, implementation, closure), frozenset(), closure),
    ):
        current = {key: source[key] for key in ("documents", "index", "preflight", "checker", "app", "artifacts")}
        current.update(branch="main", head=commits[-1]["sha"] if commits else V2737F_BASE_SHA,
                       origin=V2737F_BASE_SHA, staged=frozenset(), working=scope)
        states.append((commits, current))
    expected = (
        "authorization_prepared", "authorization_committed", "implementation_prepared",
        "implementation_committed", "closure_prepared", "closure_committed",
    ...
    positives = negatives = 0
    for (commits, current), name in zip(states, expected):
        require(validate_v2737f_facts(base, index, commits, current, base_app) == "v2737f_" + name,
                "v27.37f: positiver Übergangstest fehlgeschlagen")
        positives += 1
        # The remote may point to each actual prefix, never to an unrelated ref.
        for ref in [c["sha"] for c in commits]:
            validate_v2737f_facts(base, index, commits, {**current, "origin": ref}, base_app)
            positives += 1

    def blocked(commits, current):
        nonlocal negatives
        try:
            validate_v2737f_facts(base, index, commits, current, base_app)
        except ValidationError:
            negatives += 1
        else:
            raise ValidationError("v27.37f: isolierte Manipulation nicht blockiert")
    ...
    for commits, current in states:
        for change in (
            ...
            {"index": current["index"] + "<script>unexpected()</script>\n"},
            {"preflight": current["preflight"].replace("V2737F_IMPLEMENTATION_CHECKER", "BYPASS")},
            {"documents": tuple(d.replace("Task-ID: v27.37f", "Task-ID: v27.99")
                                .replace("Task-ID: NONE", "Task-ID: v27.99")
                                for d in current["documents"])},
            {"documents": (current["documents"][0] + "historical drift\n",
                           *current["documents"][1:])},
    ...
    commits, current = states[-1]
    blocked(commits + (closure,), current)  # repeated closure / unknown successor
    blocked(commits, {**current, "documents": auth, "working": V2737F_GATE_FILES})
    blocked(commits, {**current, "preflight": current["preflight"] + "# bypass\n"})
    blocked(commits, {**current, "checker": checker + "# changed\n"})
    blocked(commits, {**current, "index": current["index"].replace(
        V2737F_INDEX_LINE, V2737F_INDEX_LINE.replace('data-enabled="false"', 'data-enabled="true"'))})
    blocked(commits, {**current, "documents": v2737f_completion_documents(auth, identities[0])})
    # A forbidden committed product edit is rejected even if the working copy cancels it.
    altered = copy.deepcopy(list(commits))
    altered[1]["index"] += "<script>cancelled-later()</script>\n"
    blocked(tuple(altered), current)
    for commits, current in states:
        blocked(commits, {**current, "app": current["app"] + "// forbidden edit\n"})
        blocked(commits, {**current, "artifacts": ("# unexpected\n", None)})
    for position in (0, 1, 2):
        altered = copy.deepcopy(list(states[-1][0]))
        altered[position]["app"] += "// cancelled forbidden edit\n"
...
V2737G_BASE_SHA = "36bfe77f64a7d183556517522c02223a391dde74"
V2737G_TITLE = 'v27.37g – Prüfungsabschluss gegen Wiederaufnahme und Doppelverbuchung absichern'
V2737G_AUTHORIZATION_HEADING = "## Autorisierter Reparaturtask v27.37g"
V2737G_DOCUMENT_PATHS = V2737D_DOCUMENT_PATHS
V2737G_GATE_FILES = frozenset(V2737D_GATE_FILE_ORDER)
V2737G_IMPLEMENTATION_FILE_ORDER = (
    "app.js",
    "tools/check-written-exam-completion-v2737g.py",
    "docs/WRITTEN_EXAM_COMPLETION_REPAIR_V2737G.md",
    "tools/preflight.py",
)
V2737G_IMPLEMENTATION_FILES = frozenset(V2737G_IMPLEMENTATION_FILE_ORDER)
V2737G_CLOSURE_FILES = frozenset(V2737G_DOCUMENT_PATHS)
V2737G_AUTHORIZED_TASK_FIELDS = {
    **V2737F_CLOSED_TASK_FIELDS,
    "Task-ID": "v27.37g", "Status": "AUTHORIZED", "Autorisiert": "JA",
    "Titel": V2737G_TITLE,
    "Erlaubte Implementierungsdateien": ", ".join(
        f"`{path}`" for path in V2737G_IMPLEMENTATION_FILE_ORDER
    ),
}
...
V2737G_APP_LINE = '<script src="app.js?v=24.8"></script>\n'
V2737G_CHECKER_REGISTRATION = "V2737G_IMPLEMENTATION_CHECKER = None\n"
V2737G_REGISTERED_CHECKER = (
    'V2737G_IMPLEMENTATION_CHECKER = '
    '"tools/check-written-exam-completion-v2737g.py"\n'
)

V2737G_PHASES = frozenset("v2737g_" + name for name in (
    "authorization_prepared", "authorization_committed", "implementation_prepared",
    "implementation_committed", "closure_prepared", "closure_committed",
))
V2737G_FROZEN_REGRESSION_PATHS = (
    "index.html",
    "data/supabase-participant-access-adapter.js",
    "data/supabase-participant-access-bootstrap-bridge.js",
    "data/supabase-participant-access-browser-provider.js",
...
def v2737g_implementation_preflight(gate_preflight: str) -> str:
    require(gate_preflight.count(V2737G_CHECKER_REGISTRATION) == 1
            and V2737G_REGISTERED_CHECKER not in gate_preflight,
            "v27.37g: uneindeutige Loader-Checker-Registrierung im Gate")
    return gate_preflight.replace(V2737G_CHECKER_REGISTRATION, V2737G_REGISTERED_CHECKER)


def v2737g_classify_phase(
    roles: tuple[str, ...], working: frozenset[str], task_fields: dict[str, str],
) -> str:
    cases = (
        ((), V2737G_GATE_FILES, V2737G_AUTHORIZED_TASK_FIELDS, "authorization_prepared"),
        (("gate",), frozenset(), V2737G_AUTHORIZED_TASK_FIELDS, "authorization_committed"),
        (("gate",), V2737G_IMPLEMENTATION_FILES, V2737G_AUTHORIZED_TASK_FIELDS,
         "implementation_prepared"),
        (("gate", "implementation"), frozenset(), V2737G_AUTHORIZED_TASK_FIELDS,
         "implementation_committed"),
        (("gate", "implementation"), V2737G_CLOSURE_FILES, V2737G_CLOSED_TASK_FIELDS,
         "closure_prepared"),
        (("gate", "implementation", "closure"), frozenset(), V2737G_CLOSED_TASK_FIELDS,
         "closure_committed"),
    )
    matches = [phase for history, scope, fields, phase in cases
               if roles == history and working == scope and task_fields == fields]
    require(len(matches) == 1, "v27.37g: unzulässiger Task-, Scope- oder Phasenübergang")
    return "v2737g_" + matches[0]


def v2737g_expected_index(base_index: str) -> str:
    return base_index


def validate_v2737g_facts(
    base_documents: tuple[str, ...], base_index: str,
    commits: tuple[dict, ...], current: dict, base_app: str,
) -> str:
    """Pure lifecycle validator, shared by real Git reads and isolated tests.
    ...
    """
    require(current["branch"] == "main" and not current["staged"],
            "v27.37g: main und leerer Index erforderlich")
    require(len(commits) <= 3, "v27.37g: unbekannter oder wiederholter Folgecommit")
    authorization = v2737g_authorization_documents(base_documents)
    parent = V2737G_BASE_SHA
    gate_preflight = None
    gate_checker = None
    roles = []
    scopes = (V2737G_GATE_FILES, V2737G_IMPLEMENTATION_FILES, V2737G_CLOSURE_FILES)
    for number, commit in enumerate(commits):
        require(commit["parents"] == (parent,),
                "v27.37g: Commit muss direkt und linear folgen")
        require(commit["paths"] == scopes[number],
                "v27.37g: Commit-Dateiumfang oder Übergangsreihenfolge verletzt")
        require(commit["sha"] not in {V2737G_BASE_SHA, *(c["sha"] for c in commits[:number])},
                "v27.37g: wiederholte Commitidentität")
        expected_documents = (authorization if number < 2 else
                              v2737g_completion_documents(authorization, commits[1]["sha"]))
        require(commit["documents"] == expected_documents,
                "v27.37g: Commit-Dokumentvertrag oder historischer Verlauf verletzt")
        require(commit["index"] == (base_index if number == 0
                                   else v2737g_expected_index(base_index)),
                "v27.37g: Index gegenüber Basis verändert")
        v2737g_validate_product_snapshot(commit, base_app, number >= 1)
        if number == 2:
            require(commit["app"] == commits[1]["app"]
                    and commit["artifacts"] == commits[1]["artifacts"],
                    "v27.37g: Implementierungsdateien in Closure verändert")
        if number == 0:
            gate_preflight = commit["preflight"]
            gate_checker = commit["checker"]
            v2737g_implementation_preflight(gate_preflight)
        else:
            require(commit["preflight"] == v2737g_implementation_preflight(gate_preflight),
                    "v27.37g: andere Preflight-Änderung als Checker-Registrierung")
            require(commit["checker"] == gate_checker,
                    "v27.37g: Continuity-Checker nach Gate verändert")
        roles.append(("gate", "implementation", "closure")[number])
        parent = commit["sha"]
    require(current["head"] == parent, "v27.37g: HEAD passt nicht zur linearen Historie")
    require(current["origin"] in {V2737G_BASE_SHA, *(c["sha"] for c in commits)},
            "v27.37g: origin/main ist kein legitimer Lifecycle-Vorfahr")
    phase = v2737g_classify_phase(
        tuple(roles), current["working"],
        v2737a_current_task_header_fields(current["documents"][1]),
    )
    closed = phase in ("v2737g_closure_prepared", "v2737g_closure_committed")
    implemented = len(commits) >= 2 or phase == "v2737g_implementation_prepared"
    require(current["documents"] == (
        v2737g_completion_documents(authorization, commits[1]["sha"]) if closed
        else authorization), "v27.37g: aktuelle Dokumente oder Historie verändert")
    require(current["index"] == (v2737g_expected_index(base_index) if implemented
                                 else base_index),
            "v27.37g: Index gegenüber Basis verändert")
    if gate_preflight is None:
        v2737g_implementation_preflight(current["preflight"])
    else:
        require(current["preflight"] == (
            v2737g_implementation_preflight(gate_preflight) if implemented
            else gate_preflight), "v27.37g: Preflight-Registrierung oder Regression verändert")
        require(current["checker"] == gate_checker,
                "v27.37g: bestehende Lifecycle-Kontrolle nach Gate verändert")
    v2737g_validate_product_snapshot(current, base_app, implemented)
    if len(commits) >= 2:
        require(current["app"] == commits[1]["app"]
                and current["artifacts"] == commits[1]["artifacts"],
                "v27.37g: Implementierungsdateien nach Implementation verändert")
    return phase



def v2737g_app_remainder(source: str) -> str:
    """Freeze every byte outside the named written-exam boundaries."""
    begin = "// BEGIN v27.37g written exam completion\n"
    end = "// END v27.37g written exam completion\n"
    if begin in source or end in source:
        require(source.count(begin) == source.count(end) == 1,
                "v27.37g: uneindeutiger Hilfsblock")
        left, tail = source.split(begin)
        block, right = tail.split(end)
        require(right.startswith("function getActiveSession() {"),
                "v27.37g: Hilfsblock nicht unmittelbar vor getActiveSession")
        source = left + right
    boundaries = (
        ("function getActiveSession() {", "function saveActiveSession(session) {", 1),
        ("function saveActiveExamSession() {", "function scheduleExamSessionTimerSave() {", 1),
        ("function resumeActiveExamSession() {", "function pauseExam() {", 1),
        ("function renderDashboardResumeExamCard() {", "function formatSessionDate(isoDate) {", 1),
        ("function startExamMode(questionLimit, examTitle, examType) {",
         "function repeatCurrentExamMode() {", 1),
        ("function finishExamMode() {",
         "function isExamAnswerCorrect(question, selectedAnswersForQuestion) {", 2),
        ("function saveExamResult(result) {", "function showStatsPage() {", 1),
        ("function buildDashboardResumeStepV2735B(candidate) {",
         "function getWeakestCoveredTopicV2735B() {", 1),
    )
    for start, following, count in boundaries:
        require(source.count(start) == count and source.count(following) == 1,
                "v27.37g: uneindeutige Funktionsgrenze: " + start)
        left, tail = source.rsplit(start, 1)
        body, right = tail.split(following, 1)
        require(body.rstrip().endswith("}"), "v27.37g: Funktionsende fehlt")
        source = left + start + "\n}\n\n" + following + right
    return source


def v2737g_validate_product_snapshot(snapshot: dict, base_app: str, implemented: bool) -> None:
    if not implemented:
        require(snapshot["app"] == base_app and snapshot["artifacts"] == (None, None),
                "v27.37g: Produktimplementation vor Gate-Abschluss")
        return
    require(snapshot["app"] != base_app
            and v2737g_app_remainder(snapshot["app"]) == v2737g_app_remainder(base_app),
            "v27.37g: App-Änderung außerhalb autorisierter Prüfungsabschlussgrenzen")
    require(len(snapshot["artifacts"]) == 2
            and all(isinstance(value, str) and value.strip() and value.endswith("\n")
                    and not value.startswith("\ufeff") for value in snapshot["artifacts"]),
...
def detect_v2737g_phase(expected_working_files: frozenset[str] | None = None) -> str | None:
    ...
    if not git_is_ancestor(V2737G_BASE_SHA, head):
        ...
        return None
    if not active and head == V2737G_BASE_SHA:
        return None
    require(active, "v27.37g: unbekannter Nachfolgetask")
    require(detect_v2737f_phase(_closed_snapshot=V2737G_BASE_SHA)
            == "v2737f_closure_committed", "v27.37g: v27.37f nicht vollständig geschlossen")
    ...
    def snapshot(ref):
        ...
        artifacts = tuple(
            ...
            for path in V2737G_IMPLEMENTATION_FILE_ORDER[1:3]
        )
        return {
            "documents": tuple(content(ref, p) for p in V2737G_DOCUMENT_PATHS),
            "index": content(ref, "index.html"),
            "app": content(ref, "app.js"), "artifacts": artifacts,
            "preflight": content(ref, "tools/preflight.py"),
            "checker": content(ref, CHECKER_RELATIVE_PATH),
    ...
    refs = run_git(["rev-list", "--reverse", V2737G_BASE_SHA + ".." + head]).splitlines()
    require(len(refs) <= 3, "v27.37g: mehr als Gate, Implementation und Closure")
    commits = []
    previous = V2737G_BASE_SHA
    for ref in refs:
        commit = snapshot(ref)
        commit.update(
            sha=ref, parents=tuple(run_git(["rev-list", "--parents", "-n", "1", ref]).split()[1:]),
    ...
    phase = validate_v2737g_facts(
        ...
        content(V2737G_BASE_SHA, "index.html"), tuple(commits), current,
...
def run_v2737g_lifecycle_self_checks() -> tuple[int, int]:
    """Exercise transitions and document/index contracts entirely in memory."""
    import copy
    base = tuple(read_v2735f_commit_document(V2737G_BASE_SHA, p)
                 for p in V2737G_DOCUMENT_PATHS)
    index = read_v2735f_commit_document(V2737G_BASE_SHA, "index.html")
    base_app = read_v2735f_commit_document(V2737G_BASE_SHA, "app.js")
    app_fixture = base_app.replace("function getActiveSession() {",
        "// BEGIN v27.37g written exam completion\n"
        "// isolated fixture, no product implementation\n"
        "// END v27.37g written exam completion\nfunction getActiveSession() {")
    auth = v2737g_authorization_documents(base)
    identities = [hashlib.sha1(("isolated-v2737g-" + role).encode()).hexdigest()
                  for role in ("gate", "implementation", "closure")]
    preflight = V2737G_CHECKER_REGISTRATION + "# existing regression checks\n"
    checker = "# unchanged lifecycle checker\n"
    gate = dict(sha=identities[0], parents=(V2737G_BASE_SHA,),
                paths=V2737G_GATE_FILES, documents=auth, index=index,
                preflight=preflight, checker=checker, app=base_app, artifacts=(None, None))
    implementation = dict(
        sha=identities[1], parents=(identities[0],), paths=V2737G_IMPLEMENTATION_FILES,
        documents=auth, index=v2737g_expected_index(index),
        preflight=v2737g_implementation_preflight(preflight), checker=checker,
        app=app_fixture, artifacts=("# isolated checker\n", "# isolated document\n"),
    )
    closure = dict(
        sha=identities[2], parents=(identities[1],), paths=V2737G_CLOSURE_FILES,
        documents=v2737g_completion_documents(auth, identities[1]),
        index=implementation["index"], preflight=implementation["preflight"], checker=checker,
        app=app_fixture, artifacts=implementation["artifacts"],
    )
    states = []
    for commits, scope, source in (
        ((), V2737G_GATE_FILES, gate),
        ((gate,), frozenset(), gate),
        ((gate,), V2737G_IMPLEMENTATION_FILES, implementation),
        ((gate, implementation), frozenset(), implementation),
        ((gate, implementation), V2737G_CLOSURE_FILES, closure),
        ((gate, implementation, closure), frozenset(), closure),
    ):
        current = {key: source[key] for key in ("documents", "index", "preflight", "checker", "app", "artifacts")}
        current.update(branch="main", head=commits[-1]["sha"] if commits else V2737G_BASE_SHA,
                       origin=V2737G_BASE_SHA, staged=frozenset(), working=scope)
        states.append((commits, current))
    expected = (
        "authorization_prepared", "authorization_committed", "implementation_prepared",
        "implementation_committed", "closure_prepared", "closure_committed",
    ...
    positives = negatives = 0
    for (commits, current), name in zip(states, expected):
        require(validate_v2737g_facts(base, index, commits, current, base_app) == "v2737g_" + name,
                "v27.37g: positiver Übergangstest fehlgeschlagen")
        positives += 1
        # The remote may point to each actual prefix, never to an unrelated ref.
        for ref in [c["sha"] for c in commits]:
            validate_v2737g_facts(base, index, commits, {**current, "origin": ref}, base_app)
            positives += 1

    def blocked(commits, current):
        nonlocal negatives
        try:
            validate_v2737g_facts(base, index, commits, current, base_app)
        except ValidationError:
            negatives += 1
        else:
            raise ValidationError("v27.37g: isolierte Manipulation nicht blockiert")
    ...
    for commits, current in states:
        for change in (
            ...
            {"working": current["working"] | {"foreign.txt"}},
            {"index": current["index"] + "<script>unexpected()</script>\n"},
            {"preflight": current["preflight"].replace("V2737G_IMPLEMENTATION_CHECKER", "BYPASS")},
            {"documents": tuple(d.replace("Task-ID: v27.37g", "Task-ID: v27.99")
                                .replace("Task-ID: NONE", "Task-ID: v27.99")
                                for d in current["documents"])},
            {"documents": (current["documents"][0] + "historical drift\n",
                           *current["documents"][1:])},
    ...
    commits, current = states[-1]
    blocked(commits + (closure,), current)  # repeated closure / unknown successor
    blocked(commits, {**current, "documents": auth, "working": V2737G_GATE_FILES})
    blocked(commits, {**current, "preflight": current["preflight"] + "# bypass\n"})
    blocked(commits, {**current, "checker": checker + "# changed\n"})
    blocked(commits, {**current, "index": current["index"].replace(
        V2737G_INDEX_LINE, V2737G_INDEX_LINE.replace('data-enabled="false"', 'data-enabled="true"'))})
    blocked(commits, {**current, "documents": v2737g_completion_documents(auth, identities[0])})
    # A forbidden committed product edit is rejected even if the working copy cancels it.
    altered = copy.deepcopy(list(commits))
    altered[1]["index"] += "<script>cancelled-later()</script>\n"
    blocked(tuple(altered), current)
    ...
    for commits, current in states:
        for before, after in (
            ("async function initAppBoot() {", "async function initAppBoot() {\n// forbidden"),
            ("async function initAuthFlow() {", "async function initAuthFlow() {\n// forbidden"),
            ("function finishExamMode() {", "function finishExamMode() {\n// obsolete edit"),
        ):
            blocked(commits, {**current, "app": current["app"].replace(before, after, 1)})
    return positives, negatives
...
def main() -> int:
    try:
        ...
        v2737f_self_checks = None
        if V2737F_AUTHORIZATION_HEADING in state_text:
            v2737f_self_checks = run_v2737f_lifecycle_self_checks()
        ...
        manipulation_checks += v2737c_manipulation_checks
        v2737b_completion_checks = 0
        if v2737b_phase in {"v2737b_closure_prepared", "v2737b_closure_committed"}:
            implementation_commit = v2737b_implementation_commit
            require(
                implementation_commit is not None,
    ...
    print("Projektkontinuität, abgeschlossene v27.37a und enger v27.37b-Nachfolgelifecycle: OK")
    ...
    if v2737d_phase is not None:
        if v2737d_phase in V2737G_PHASES:
            ...
            print("Historischer v27.37e-Abschluss: vollständig validiert / PASS")
            print(f"Aktuelle v27.37f-Phase: {v2737d_phase}")
        elif v2737d_phase.startswith("v2737e_"):
    ...
    if v2737f_self_checks is not None:
        print(f"v27.37f isolierter Lifecycle: {v2737f_self_checks[0]} Positivtests / PASS; "
              f"{v2737f_self_checks[1]} Negativtests / vollständig blockiert; keine Git-Mutation")
    ...
    print(
        "v27.37b-GATE-BOOTSTRAP-/Nachfolgeprofil: "
    ...
    print(
        ...
        f"v27.37b-GATE-BOOTSTRAP: {v2737b_manipulation_checks})"
    )
    return 0
